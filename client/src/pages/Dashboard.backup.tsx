...
const safelyHandleError = async (operation: () => Promise<any>, errorMessage: string) => {
  try {
...
export default function Dashboard() {
  ...
  const loadMedications = useCallback(async () => {
    ...
    if (!auth.currentUser) {
      console.log("No authenticated user, skipping medication fetch");
      return;
    }
    ...
    try {
      ...
      if (!snapshot.empty) {
        ...
        const medicationsData = snapshot.docs.map((doc: any) => {
          const data = doc.data();
          return {
            id: doc.id,
            name: data.name || '',
        ...
        console.log("Medications state updated with", medicationsData.length, "items");
      } else {
      ...
      // Don't show error toast for empty results or on home page refresh
      // Only show for actual connection/database errors
      if (error instanceof Error && 
          error.message !== 'No data found' && 
          (error.message.includes('network') || 
          error.message.includes('permission'))) {
        toast.error('Failed to load medications data');
      }
    }
  }, []);
  ...
  const loadTodayWaterLogs = useCallback(async () => {
    ...
    if (!auth.currentUser) {
      console.log("No authenticated user, skipping water log fetch");
      return;
    }
    ...
    // Calculate today's date bounds (start and end of day)
    ...
    try {
      ...
      const snapshot = await getDocs(waterLogsQuery);
      
      // Always set water intake, even if no data found
      let total = 0;
      const logs: WaterLog[] = [];
        
      if (!snapshot.empty) {
        console.log(`Found ${snapshot.size} water logs for today`);
        ...
        console.log("No water logs found for today");
      }
      ...
      console.log(`Water intake updated: total=${total}ml, logs=${logs.length}`);
    } catch (error) {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged((user) => {
      if (user) {
        ...
        try {
          // Load only essential data first, then load the rest with a delay
          loadUserProfile();
          
          // Delay loading of non-critical data
          setTimeout(() => {
            try {
              loadTodayWaterLogs();
            } catch (error) {
          ...
          // Further delay loading of reminders and medications
          setTimeout(() => {
            try {
              loadMedications();
            } catch (error) {
    ...
    return () => unsubscribe();
  }, [loadTodayWaterLogs, loadMedications, loadUserProfile]);
  
  // If mobile view, render the mobile dashboard - use standard breakpoint of 768px
  ...
  // Force update the state for medications modal
  useEffect(() => {
    // Force initial render of modals
    setTimeout(() => {
      if (typeof setShowMedicationsModal === 'function') {
        setShowMedicationsModal(false);
        setShowWaterIntakeModal(false);
  ...
  // Fix the loadReminders issue by removing existing duplicate definitions
  // and properly defining it once at the class level
  const loadRemindersData = async () => {
    try {
      if (!auth.currentUser) return;
      
      const remindersRef = collection(db, "reminders");
      const q = query(remindersRef, where("userId", "==", auth.currentUser.uid));
  ...
  // Update the useEffect to call loadReminders
  useEffect(() => {
    loadRemindersData();
    // ... existing code ...
  }, [loadTodayWaterLogs, loadMedications, loadUserProfile]);
  ...
  // Modify the medication event handler
  const handleAddMedication = async (medicationData: Omit<Medication, 'id' | 'userId' | 'createdAt'>) => {
    if (!auth.currentUser) {
    ...
    try {
      // Create medication object with correct typing
      const newMedication = {
        ...medicationData,
        userId: auth.currentUser.uid,
        id: uuidv4(),
      ...
      await addDoc(medicationsCollectionRef, newMedication);
  ...
  // Modify the loadMedications function
  useEffect(() => {
  ...
  // Add this at the top of the component to handle button clicks safely
  const safeButtonHandler = (callback: () => void) => {
          ...
          >
            ...
            <div className="flex gap-2 mt-auto">
              <Button 
                ...
                onClick={safeButtonHandler(() => safelyOpenModal(setShowMedicationsModal))}
              >
              ...
              <Button
                ...
                onClick={safeButtonHandler(() => safelyOpenModal(setShowMedicationsHistory))}
              >
          ...
          >
            ...
            <div className="flex gap-2 mt-auto">
              <Button 
                ...
                onClick={safeButtonHandler(() => safelyOpenModal(setShowWaterIntakeModal))}
              >
                <Plus className="w-4 h-4 mr-2" />
                Log Intake
              </Button>
